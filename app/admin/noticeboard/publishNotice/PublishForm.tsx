...
export default function NoticeboardForm() {
  ...
  const handleImageDelete = (previewUrlToDelete: string) => {
    const imageToDelete = images.find(img => img.previewUrl === previewUrlToDelete);
    if (imageToDelete) {
      URL.revokeObjectURL(imageToDelete.previewUrl); // Clean up memory
    }
    setImages(prev => prev.filter(img => img.previewUrl !== previewUrlToDelete));
  };
  ...
  const handleCopyId = (previewUrlToCopy: string) => {
    ...
    setTimeout(() => {
        setImages(prev => prev.map(img => 
            img.previewUrl === previewUrlToCopy ? { ...img, copySuccess: false } : img
        ));
    }, 2000);
  };
  ...
  return (
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      {['title', 'description'].map((field) => (
        <div key={field}>
          <Label htmlFor={field} className="block font-semibold text-sm font-medium text-gray-700 capitalize">{field}</Label>
          <Input
            id={field}
            name={field}
            placeholder={field}
      ...
      <div className="flex flex-col md:flex-row md:space-x-6 md:items-start space-y-6 md:space-y-0">
        ...
        <div className="md:w-1/2 space-y-6">
          <div>
            <Label htmlFor="location" className="block text-sm font-semibold font-medium text-gray-700">Location</Label>
            <Input id="location" name="location" placeholder='Location' type="text" value={formData.location} onChange={handleChange} className="mt-1 w-full px-4 py-2 border border-gray-300 rounded-lg text-gray-900 placeholder:text-gray-400" required/>
          </div>
          <div>
            <Label htmlFor="eventTime" className="block text-sm font-semibold font-medium text-gray-700">Time</Label>
            <Input id="eventTime" name="eventTime" placeholder='Event Time' type="datetime-local" value={formData.eventTime} onChange={handleChange} className="mt-1 w-full px-4 py-2 border border-gray-300 rounded-lg text-gray-900 placeholder:text-gray-400" required />
          </div>
        </div>
        
        {/* Scrollable Multi-Image Preview Pane */}
        <div className="md:w-1/2">
          <Label className="block text-sm font-semibold font-medium text-gray-700">Images</Label>
          ...
          <div className="mt-1 h-48 w-full p-2 border-2 border-dashed border-gray-300 rounded-lg overflow-y-auto flex flex-wrap gap-4">
            ...
            {images.map((image) => (
              <div key={image.previewUrl} className="relative w-28 h-28 group flex-shrink-0">
                {/* {image.isUploading && (
                ...
                {/*!image.isUploading && */image.id && (
                    <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity bg-black bg-opacity-40 rounded-lg flex items-center justify-center space-x-2">
                      <button type="button" onClick={() => handleCopyId(image.previewUrl)} className="p-1.5 bg-white rounded-full text-black">
                          <CopyIcon />
                      </button>
                    </div>
                )}
                  <button type="button" onClick={() => handleImageDelete(image.previewUrl)} className="absolute top-1 right-1 z-20 bg-black bg-opacity-50 text-white rounded-full p-1 leading-none hover:bg-opacity-75">
                      &#x2715;
                  </button>
              </div>
            ))}
            ...
            <div
              ...
              className="w-28 h-28 flex-shrink-0 flex flex-col items-center justify-center border-2 border-transparent rounded-lg cursor-pointer bg-gray-100 hover:bg-gray-200 text-gray-500"
            >
              <UploadIcon />
      ...
      <div>
        <Label className="block text-sm font-medium font-semibold text-gray-700">Body (Markdown Supported)</Label>
