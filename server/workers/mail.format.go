...
func formatVerificationEmail(job MailJob) (MailContent, error) {
	...
	mailTemplate := fmt.Sprintf(`<h2>Thank you for signing up!</h2> 
		<p>Bellow is your otp for verification:</p>
		<h2>%s</h2>
		<p>or</p>
		<p>you may directly verify your email by clicking the link below:</p>
		<p><a href="%s">Verify Email</a></p>
		<p>This link is valid for next %d hours</p>
		<p>If this action was not taken by you, please ignore this mail, and do not share this opt with anyone</p>`, token, link, expiry)
...
func formatThanksEmail(job MailJob) (MailContent, error) {
	...
	username := job.Data["username"]
	contentTitle := job.Data["content_title"]
	tmpl :=fmt.Sprintf(`<h2>Hi %s</h2>
		<p>Thank you for your contribution: <strong>%s</strong>.</p>
		<p>We appreciate your involvement in the community!</p>`, username, contentTitle)
	body.Write([]byte(tmpl))
...
func formatWarningEmail(job MailJob) (MailContent, error) {
	...
	tmpl :=fmt.Sprintf(`<h2>Hi %s</h2>
		<p>We've found that one of your recent submissions violated our community guidelines.</p>
		<p>Reason: %s</p>
		<p>Please make sure to follow the rules to avoid further action.</p>`, username, reason)
...
func formatPublishNotice(job MailJob) (MailContent, error) {
	...
	title := job.Data["title"]
	var body bytes.Buffer
	tmpl :=fmt.Sprintf(`<h2>Hello %s,</h2>
		<p>Your content titled <strong>%s</strong> has been successfully published!</p>
		<p>Thanks for being an active part of our community.</p>`, username, title)
	body.Write([]byte(tmpl))
	return MailContent{
		To:      job.To,
		Subject: "Your content has been published!",
...
func formatAccountDeletionEmail(job MailJob) (MailContent, error) {
	...
	tmpl :=fmt.Sprintf(`<h2>Hello %s,</h2>
		<p>Your account has been deleted because it was not verified within 24 hours of creation.</p>
		<p>If you wish to use our services, please sign up again.</p>`, username)
	body.Write([]byte(tmpl))
	return MailContent{
		To:      job.To,
		Subject: "Account Deleted - Verification Timeout",
